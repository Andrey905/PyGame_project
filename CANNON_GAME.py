...
fire_surf_1 = pygame.image.load('fire_for_shoot.png')
fire_surf_2 = pygame.image.load('fire_for_shoot_2.png')

ground_scale = pygame.transform.scale(ground_surf,
                                      (int(ground_surf.get_width() // 1), int(ground_surf.get_height() // 1)))
background_scale = pygame.transform.scale(background_surf, (int(background_surf.get_width() // 1),
                                                            int(background_surf.get_height() // 1)))
fire_scale_1 = pygame.transform.scale(fire_surf_1,(int(fire_surf_1.get_width() // 14),
                                                   int(fire_surf_1.get_height() // 21)))
fire_scale_2 = pygame.transform.scale(fire_surf_2, (int(fire_surf_2.get_width() // 14),
                                                    int(fire_surf_2.get_height() // 19)))
...
def spawn_new_falling_thing(game_state):
    ...
    if min_f_th_n <= 10 and len(game_state.things_list) == 1:
        game_state.things_list.append(falling_things.Falling_things(WIDTH, HEIGHT - GROND_HEIGHT, 'stone.png',
                                                                    game_state.game_progress.game_level,
                                                                    game_state.game_progress.cannon_level))
        return True
    else:
        return False
...
def restart_game(game_state):
    ...
    game_state.things_list.append(falling_things.Falling_things(WIDTH, HEIGHT - GROND_HEIGHT, 'stone.png', game_state.game_progress.game_level, game_state.game_progress.cannon_level))
...
start_window = game_windows.Start_window(screen, WIDTH, HEIGHT, 'Cannon_for_game_windows.png', FPS, cannon_level)
finish_window = None
FPS_counter = 0
