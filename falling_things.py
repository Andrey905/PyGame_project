...
class Falling_things():
    def __init__(self, SCREEN_WIDTH, SCREEN_HEIGHT, picture, game_level, cannon_level):
        ...
        file = os.path.join(os.getcwd(), "stone.png")
        #print("file ", file)
        self.__image = pygame.image.load(file)
        #print("loaded ", file)
        #self.__image = pygame.image.load(picture)  #
        self.__scale_image = pygame.transform.scale(self.__image, (
            int(self.__image.get_width() // 6), int(self.__image.get_height() // 6)))
        self.__rect = self.__scale_image.get_rect(
            center=[random.randint(50, self.__screen_width - 50), -50])
        ...
        self.__fontobj = pygame.font.Font('18547.ttf', self.__font_size)
        self.__txt_color = (
            self.__txt_color_index_red, self.__txt_color_index_green, self.__txt_color_index_blue)
    ...
    def hit(self):
        self.__txt_color_index_red = self.__clamp_zero(
            self.__txt_color_index_red - self.__txt_color_delta_red)
        self.__txt_color_index_green = self.__clamp_zero(
            self.__txt_color_index_green - self.__txt_color_delta_green)
        self.__txt_color_index_blue = self.__clamp_zero(
            self.__txt_color_index_blue - self.__txt_color_delta_blue)
        self.__txt_color = (
            self.__txt_color_index_red, self.__txt_color_index_green, self.__txt_color_index_blue)
    ...
    def check_cross_bullet_and_thing(self, bullet):
        ...
        return is_inside_circle(bullet_param[0], bullet_param[1], self.__rect.center,
                                self.__cir_circle_r)
    ...
    def __reduce_image(self, reduce_index):
        self.__scale_image = pygame.transform.scale(self.__image, (
            int(self.__image.get_width() // reduce_index),
            int(self.__image.get_height() // reduce_index)))
    ...
    def __make_rotated_image(self):
        return pygame.transform.rotate(self.__scale_image,
                                       self.__rotate_angle), pygame.transform.rotate(
            self.__txt_image, self.__rotate_angle)
